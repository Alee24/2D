...
1. A domain name (`www.secondesk.ke` and `secondesk.ke`) pointing to your VPS public IP address via DNS A records.
2. Inbound port `80` and `443` open on your VPS firewall (e.g. UFW or security groups).

## Easy One-Click Setup

To deploy the application, SSH into your VPS and run the following command:
...
2. **Creates directory `/var/www/sd`** and clones the project source files into it.
...
All application containers run under Docker Compose project name `sd`. Navigate to the deployment folder `/var/www/sd/deployment` to run control commands:

- **Check logs**:
  ```bash
  docker compose -p sd logs -f
  ```
- **Stop application**:
  ```bash
  docker compose -p sd down
  ```
- **Restart application**:
  ```bash
  docker compose -p sd restart
  ```
- **Rebuild and Update application**:
  ```bash
  docker compose -p sd up -d --build
  ```
